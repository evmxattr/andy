...
def root_device(device=None):
    ...
    commands.push(os.path.join(os.path.expanduser('~/.andy'), 'binaries/su/x86/su.pie'), '/system/xbin/su', device)
...
def install_xposed(device=None):
    ...
    commands.install(os.path.join(os.path.expanduser('~/.andy'),'framework/xposed/installer/%s' % installer.get(api, 'xposed.installer_3.1.2.apk')), device)
    if api > 19:
        xposed_dir = 'xposed-v88-sdk%s-x86' % api
        commands.push(os.path.join(os.path.expanduser('~/.andy'), 'framework/xposed/sdk/%s' % xposed_dir), '/data/local/tmp/', device)
...
def install_busybox(device=None):
    ...
    commands.push(os.path.join(os.path.expanduser('~/.andy'), 'binaries/busybox/x86/busybox'), '/system/xbin/', device)
...
def install_frida(device=None):
    ...
    commands.push(os.path.join(os.path.expanduser('~/.andy'), 'binaries/frida/x86/frida-server'), '/data/local/tmp/', device)
...
def install_apps(device=None):
    ...
    for (dirpath, dirnames, filenames) in os.walk(os.path.expanduser('~/.andy')):
